...
namespace Hahn.ApplicatonProcess.July2021.Domain.Validators
{
    public class UserValidator : AbstractValidator<UserDTO>
    {
        public UserValidator()
        {
            ...
            RuleFor(x => x.Email).NotNull().NotEmpty().Matches("@(.)+(?:[A-Z]{2}|com|org|net|do|gov|biz|info|mobi|name|aero|jobs|museum|tv)\\b").WithMessage("The email must have an @ and a domain such as: .com, .org, .net, .do, .gov, .biz, .info, .mobi, .name, .aero, .jobs, .museum, .tv");
